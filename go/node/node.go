...
func (buffer *nodeArchiveBuffer) Write(data []byte) (
	int,
	error,
) {
...
func (node *Node) LoadTar(data []byte) error { // legacy error contract
...
func dataFileFromTarEntry(tarReader *tar.Reader, header *tar.Header) (
	*dataFile,
	error,
) {
...
func (node *Node) Walk(root string, walkFunc fs.WalkDirFunc, options WalkOptions) error { // legacy error contract
...
func walkFilterDecision(entryPath string, entry fs.DirEntry, err error, filter func(string, fs.DirEntry) bool) (
	bool,
	error,
) {
...
func (node *Node) ReadFile(name string) (
	[]byte,
	error,
) {
	name = core.TrimPrefix(name, "/")
...
func (node *Node) ExportFile(sourcePath, destinationPath string, permissions fs.FileMode) error { // legacy error contract
...
func (node *Node) CopyTo(target coreio.Medium, sourcePath, destinationPath string) error { // legacy error contract
...
func (node *Node) Open(name string) (
	fs.File,
	error,
) {
	name = core.TrimPrefix(name, "/")
...
func (node *Node) Stat(name string) (
	fs.FileInfo,
	error,
) {
	name = core.TrimPrefix(name, "/")
...
func (node *Node) ReadDir(name string) (
	[]fs.DirEntry,
	error,
) {
	name = core.TrimPrefix(name, "/")
...
func (node *Node) Read(filePath string) (
	string,
	error,
) {
...
// Example: _ = nodeTree.Write("config/app.yaml", "port: 8080")
func (node *Node) Write(filePath, content string) error { // legacy error contract
...
func (node *Node) WriteMode(filePath, content string, mode fs.FileMode) error { // legacy error contract

	return node.Write(filePath, content)
}

// Example: _ = nodeTree.EnsureDir("config")
func (node *Node) EnsureDir(directoryPath string) error { // legacy error contract
...
func (node *Node) Delete(filePath string) error { // legacy error contract
...
func (node *Node) DeleteAll(filePath string) error { // legacy error contract
...
func (node *Node) Rename(oldPath, newPath string) error { // legacy error contract
...
func (node *Node) List(filePath string) (
	[]fs.DirEntry,
	error,
) {
	filePath = core.TrimPrefix(filePath, "/")
...
func (node *Node) Create(filePath string) (
	goio.WriteCloser,
	error,
) {
	filePath = core.TrimPrefix(filePath, "/")
...
func (node *Node) Append(filePath string) (
	goio.WriteCloser,
	error,
) {
	filePath = core.TrimPrefix(filePath, "/")
...
func (node *Node) ReadStream(filePath string) (
	goio.ReadCloser,
	error,
) {
...
func (node *Node) WriteStream(filePath string) (
	goio.WriteCloser,
	error,
) {
...
func (writer *nodeWriter) Write(data []byte) (
	int,
	error,
) {
...
func (writer *nodeWriter) Close() error { // legacy error contract
...
func (reader *dataFileReader) Read(buffer []byte) (
	int,
	error,
) {
...
func (directory *dirFile) Read([]byte) (
	int,
	error,
) {
