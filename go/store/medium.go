...
// Example: medium, _ := store.NewMedium(store.Options{Path: "config.db"})
// Example: _ = medium.Write("app/theme", "midnight")
func NewMedium(options Options) (
	*Medium,
	error,
) {
...
func (medium *Medium) Read(entryPath string) (
	string,
	error,
) {
...
func (medium *Medium) Write(entryPath, content string) error { // legacy error contract
...
func (medium *Medium) WriteMode(entryPath, content string, mode fs.FileMode) error { // legacy error contract

	return medium.Write(entryPath, content)
}

// Example: _ = medium.EnsureDir("app")
func (medium *Medium) EnsureDir(entryPath string) error { // legacy error contract
...
func (medium *Medium) Delete(entryPath string) error { // legacy error contract
...
func (medium *Medium) DeleteAll(entryPath string) error { // legacy error contract
...
// Example: entries, _ := medium.List("app")
func (medium *Medium) List(entryPath string) (
	[]fs.DirEntry,
	error,
) {
	group, key := splitGroupKeyPath(entryPath)
...
func (medium *Medium) Stat(entryPath string) (
	fs.FileInfo,
	error,
) {
	group, key := splitGroupKeyPath(entryPath)
...
func (medium *Medium) Open(entryPath string) (
	fs.File,
	error,
) {
	group, key := splitGroupKeyPath(entryPath)
...
func (medium *Medium) Create(entryPath string) (
	goio.WriteCloser,
	error,
) {
	group, key := splitGroupKeyPath(entryPath)
...
func (medium *Medium) Append(entryPath string) (
	goio.WriteCloser,
	error,
) {
	group, key := splitGroupKeyPath(entryPath)
...
func (medium *Medium) ReadStream(entryPath string) (
	goio.ReadCloser,
	error,
) {
	group, key := splitGroupKeyPath(entryPath)
...
func (medium *Medium) WriteStream(entryPath string) (
	goio.WriteCloser,
	error,
) {
...
func (entry *keyValueDirEntry) Info() (
	fs.FileInfo,
	error,
) {
...
func (file *keyValueFile) Read(buffer []byte) (
	int,
	error,
) {
...
func (writer *keyValueWriteCloser) Write(data []byte) (
	int,
	error,
) {
...
func (writer *keyValueWriteCloser) Close() error { // legacy error contract
