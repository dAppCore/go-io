...
func (keyValueStore *KeyValueStore) Close() error { // legacy error contract

	return keyValueStore.database.Close()
}

// Example: theme, _ := keyValueStore.Get("app", "theme")
func (keyValueStore *KeyValueStore) Get(group, key string) (
	string,
	error,
) {
	var value string
...
// Example: _ = keyValueStore.Set("app", "theme", "midnight")
func (keyValueStore *KeyValueStore) Set(group, key, value string) error { // legacy error contract
...
func (keyValueStore *KeyValueStore) Delete(group, key string) error { // legacy error contract
...
func (keyValueStore *KeyValueStore) Count(group string) (
	int,
	error,
) {
	var count int
...
func (keyValueStore *KeyValueStore) DeleteGroup(group string) error { // legacy error contract
...
func (keyValueStore *KeyValueStore) ListGroups() (
	[]string,
	error,
) {
...
func (keyValueStore *KeyValueStore) GetAll(group string) (
	map[string]string,
	error,
) {
...
func (keyValueStore *KeyValueStore) Render(templateText, group string) (
	string,
	error,
) {
	rows, err := keyValueStore.database.Query("SELECT entry_key, entry_value FROM entries WHERE group_name = ?", group)
