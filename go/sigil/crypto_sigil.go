...
func NewChaChaPolySigil(key []byte, nonce any) (
	*ChaChaPolySigil,
	error,
) {
	if len(key) != 32 {
...
func (s *ChaChaPolySigil) In(data []byte) (
	[]byte,
	error,
) {
	if s.key == nil {
...
func (s *ChaChaPolySigil) Out(data []byte) (
	[]byte,
	error,
) {
	if s.key == nil {
...
func (s *ChaChaPolySigil) newAEAD() (
	cipher.AEAD,
	error,
) {
...
func NonceFromCiphertext(ciphertext []byte) (
	[]byte,
	error,
) {
	nonceSize := chacha20poly1305.NonceSizeX
