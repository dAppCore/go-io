...
func readAction(_ context.Context, opts core.Options) core.Result {
	...
	if !ok {
		return core.Result{}.New(core.E("webdav.readAction", "medium is required", fs.ErrInvalid))
	}
...
func writeAction(_ context.Context, opts core.Options) core.Result {
	medium, ok := opts.Get("medium").Value.(*Medium)
	if !ok {
		return core.Result{}.New(core.E("webdav.writeAction", "medium is required", fs.ErrInvalid))
	}
